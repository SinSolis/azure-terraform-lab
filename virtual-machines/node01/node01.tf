...
resource "azurerm_network_interface" "node01nic" {
  name                = "node01nic"
  location            = "Central US"
  resource_group_name = "rglab"

  ip_configuration {
    name                          = "node01nicconfig"
...
resource "azurerm_network_interface_security_group_association" "example" {
  network_interface_id      = azurerm_network_interface.node01nic.id
  network_security_group_id = data.azurerm_network_security_group.nsglabid.id
}

# Create virtual machine
resource "azurerm_linux_virtual_machine" "node01vm" {
  name                  = "node01"
  location              = "Central US"
  resource_group_name   = "rglab"
  network_interface_ids = [azurerm_network_interface.node01nic.id]
  size                  = "Standard_DS1"
